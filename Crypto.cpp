...
#define FILENAME "StudentsData.bin"
...
void Crypt() {
	...
	command += FILENAME;
	command += " -out StudentsData.bin.enc -pass pass:\"";
	command += pass;
	command += "\" -pbkdf2";
	system(command.c_str());
	// удаляем незашифрованный файл
	if (remove(FILENAME) != 0) {
		cout << "[ERROR] - deleting database.bin" << endl;
	}
...
void Decrypt() {
	...
	command += FILENAME;
	command += ".enc -out StudentsData.bin -pass pass:\"";
	command += pass;
	command += "\" -pbkdf2";
	system(command.c_str());
