...
void Student::getBirthDate() {
	...
	while (true) {
		cout << "Введите год рождения (число от 1900 до 2008): ";
		out_year = getDigit("Введите год рождения (число от 1900 до 2008): ");
		cout << "\n";
...
void Student::editStudent(Student* student, int student_count) {
	if (student_count == 0) { cout << "Тут что-то пусто. Для начала, добавьте студентов\n"; }
		...
		while (true) {
			cout << "Введите порядковый номер студента, данные которого хотите изменить: ";
			requred_student = getDigit("Введите порядковый номер студента, данные которого хотите изменить: ");
			cout << "\n";
